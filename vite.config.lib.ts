  ...
  build: {
    lib: {
      entry: {
        ...
        "angular-hooks": resolve(__dirname, "src/angular/hooks-entry.ts"),
        ...
        "plugins/pattern-recognition": resolve(
          __dirname,
          "src/plugins/pattern-recognition/index.ts",
        ),
    ...
    rollupOptions: {
      external: (id) =>
        frameworkExternals.some((dep) => id === dep || id.startsWith(`${dep}/`)),
      output: {
        globals: {
          react: "React",
          "react-dom": "ReactDOM",
