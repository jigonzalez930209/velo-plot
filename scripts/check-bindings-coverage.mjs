...
const coveragePath = resolve("coverage/bindings/coverage-final.json");
const MIN = 98;

if (!existsSync(coveragePath)) {
  console.error(`Missing ${coveragePath}. Run pnpm test:coverage:bindings first.`);
  process.exit(1);
}
...
for (const [file, data] of Object.entries(coverage)) {
  if (!prefixes.some((p) => file.includes(p))) continue;
...
if (failures.length) {
  console.error("Bindings coverage below 98%:\n" + failures.sort().join("\n"));
  process.exit(1);
}

console.log("Bindings coverage meets 98% on all included files.");
