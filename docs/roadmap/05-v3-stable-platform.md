...
| 5.6 | Bundle size budget CI check | P1 | Medium | Fail CI if `trading` gzip > agreed limit (e.g. 150KB) |

### P0 — API stability audit

| ID | Task | Priority | Complexity | Definition of done |
|----|------|----------|------------|-------------------|
| 5.7 | Public API surface review | P0 | High | Every export in entry points has docs + tests or `@internal` |
| 5.8 | Deprecation cleanup | P0 | Medium | No deprecated APIs without removal timeline |
...
| 5.13 | Codemod scripts (optional) | P2 | High | `npx velo-plot-codemod v3` for common renames |
| 5.14 | Release candidate cycle | P0 | Low | v3.0.0-rc.1, rc.2 with community feedback window |
...
| 5.16 | Coverage threshold ≥60% | P0 | Medium | Enforced in vitest config |
| 5.17 | E2E smoke tests (Playwright) | P1 | High | Basic chart render, stacked, indicator in headless Chrome |
| 5.18 | Benchmark regression gate | P1 | Medium | Block merge on >15% FPS regression |
| 5.19 | Security audit | P1 | Low | `pnpm audit` clean or documented exceptions |
...
- [ ] Zero public API methods that throw `not implemented`
- [ ] All plugins in `PLUGIN-STATUS.md` are `complete` or explicitly `experimental`
- [ ] Test coverage ≥60% line coverage
- [ ] ESLint clean
- [ ] TypeScript strict mode with no `any` in public types

### Features (from prior stages)

- [ ] Business-day time scale
- [ ] `addIndicator()` high-level API
- [ ] Drawing tools with undo/redo
- [ ] Replay + price alerts
- [ ] Forecasting without stubs
- [ ] `<StackedPlot />` React component
- [ ] Candlestick virtualization
- [ ] Indicator worker offload
...
- [ ] All bundle entry points documented
- [ ] Trading + scientific end-to-end guides
- [ ] Known limitations section in every major API page

### CI/CD

- [ ] CI on every PR: test, lint, build
- [ ] Benchmark job (blocking or warning)
- [ ] npm publish workflow uses Node 24 actions
- [ ] Docs deploy on release tag
